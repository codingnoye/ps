...
const int seedlen = 333;
lint fibo[seedlen];
lint pow(lint b, lint n) {
    lint e = 1;
    while (n > 1) {
        if (n & 1) {
			e *= b;
...
void solve() {
	...
	fibo[0] = 1;
	fibo[1] = 1;
	for (int i=2; i<seedlen; i++) fibo[i] = (fibo[i-1] + fibo[i-2]) % mod;
	seed.push_back(A(1, k));
	for (int i=2; i<seedlen; i++) seed.push_back((int)((*(seed.end()-1) + A(i, k))%mod));
    cout<<(guess_nth_term(seed, n-1)%mod)<<endl;
}
